...
class Hdf5db:
    ...
    def getVersionInfo():
        versionInfo = {}
        versionInfo['hdf5-json-version'] = "1.1.1-jfm" # todo - have this auto-synch with package version
    ...
    def _get_next_uuid(self):
        ret = str(uuid.UUID(int=self._current_uuid))
        self._current_uuid = self._current_uuid + 1
        return ret
    ...
    def initFile(self):
        ...
        if not self.root_uuid:
            if self._use_int_uuids:
                self.root_uuid = self._get_next_uuid()
            else:
        ...
        if not data:
            ...
            for prefix in ("datasets", "groups", "datatypes"):
                if data.startswith(prefix):
                    ...
                    if len(uuid_ref) != (UUID_LEN + 1):
                        uuid_ref = '/' + self._uuid_lookup(uuid_ref[1:])
            ...
            out = []
            for item in data:
                if type(item) == h5py.h5r.Reference:
                    out.append(item)  # jfm says: Not sure why I needed to handle this case specially
                else:
    ...
    """
    def getDatasetValuesByUuid(self, obj_uuid, slices=Ellipsis, format="json", tofile=None):
        dset = self.getDatasetObjByUuid(obj_uuid)
        ...
        if dt.kind == 'O':
            ...
            if format == "json":
                ...
                if tofile:
                    values[slices].tofile(tofile)
                else:
                    values = values[slices].tobytes()
        else:
            values = dset[slices]
            ...
            if format == "json":
                ...
                if tofile:
                    # is there a more efficient way to do this? probably so.
                    values.tofile(tofile)
                else:
    ...
    def setDatasetValuesByUuid(self, obj_uuid, data, slices=None, format="json"):
        ...
        if format == "binary" and ((type(data) is not bytes) and (not _is_file_like(data))):
            msg ="data must be of type bytes or file for binary writing"
            self.log.info(msg)
            raise IOError(errno.EINVAL, msg)
        ...
        if format == "binary":
            if npoints*itemSize != _binary_data_length(data):
                msg = "Expected: " + str(npoints*itemSize) + " bytes, but got: " + str(_binary_data_length(data))
                self.log.info(msg)
                raise IOError(errno.EINVAL, msg)
            if dset.dtype.shape == ():
                if _is_file_like(data):
                    arr = np.fromfile(data, dtype=dset.dtype)
                else:
                    arr = np.fromstring(data, dtype=dset.dtype)  # jfm says: i don't understand why this was originally "fromstring"
                arr = arr.reshape(np_shape)  # conform to selection shape
            else:
                # tricy array type!
                arr = np.empty(np_shape, dtype=dset.dtype)
                if _is_file_like(data):
                    base_arr = np.fromfile(data, dtype=dset.dtype.base)
                else:
                    base_arr = np.fromstring(data, dtype=dset.dtype.base)  # jfm says: i don't understand why this was originally "fromstring"
